  ...
  home.packages = with pkgs;
    ...
    ++ (
      ...
      then [
        ...
        # GUI applications migrated from homebrew to nixpkgs (stable packages)
        # Only including packages that are confirmed to work on macOS via nixpkgs
        firefox # Stable browser, cross-platform
        # Note: Many other packages from homebrew casks are not available
        # or don't work well on macOS via nixpkgs, so they remain in homebrew
        # See modules/darwin.nix for detailed comments about which packages
        # should stay in homebrew and why
      ]
      else throw "Unsupported OS for this home-manager configuration"
    );
